...
def test_reward_to_go_accumulates_dense_rewards_backward():
    assert np.allclose(reward_to_go(np.array([1.0, -0.5, 3.0])), [3.5, 2.5, 3.0])
...
def test_batch_padding_does_not_create_valid_targets():
    ...
    assert batch.value_target[0].tolist() == [3.5, 2.5, 3.0]
