"""VPT transformer-XL recurrent core — the temporal backbone ROCKET-2 reasons with.

Vendored from OpenAI VPT (via ROCKET-1's ``rocket/arm/utils/vpt_lib/util.py``, which
is what ``minestudio.utils.vpt_lib.util`` re-exports). Trimmed to what ROCKET-2
actually uses: it builds ``ResidualRecurrentBlocks`` with ``inject_condition=False``,
so the FiLM / gated-cross-attention injection blocks — which pull in Minecraft-only
``rocket.arm.models.fusions`` — are dropped. ``store_args`` lives in a leaf module here rather
than in vendored ``minecraft_util``, whose only other content is the Minecraft
action-head factory.

Behaviour of what remains is unchanged from upstream.
"""
...
from contra_policy.vpt import torch_util as tu
from contra_policy.vpt.masked_attention import MaskedAttention
from contra_policy.vpt.store_args import store_args
from contra_policy.vpt.tree_util import tree_map


class FanInInitReLULayer(nn.Module):
    """Linear/conv layer with a fan-in init that produces std-1 outputs under ReLU.

    :param inchan: number of input channels
    :param outchan: number of output channels
    :param layer_type: "linear" (dense), "conv" (2D) or "conv3d"
    :param init_scale: multiplier on initial weights
    :param batch_norm: use batch norm after the layer (for 2D data)
    :param group_norm_groups: if set, use group norm with this many groups
    ...
    def forward(self, x):
        """Norm before the layer — upstream found this slightly better for both IAM and BC."""
...
class ResidualRecurrentBlocks(nn.Module):
    """A stack of ``n_block`` transformer-XL (or LSTM) blocks with carried memory.

    ``forward(x, first, state)`` takes ``x`` of shape (B, T, hidsize), a boolean
    (B, T) ``first`` marking episode starts, and a flat list of ``3 * n_block``
    state tensors (transformer) or ``n_block`` entries (LSTM). Returns the same
    shapes plus the updated state.
    """
    ...
    def forward(self, x, first, state, **kwargs):
        ...
        if "lstm" in self.recurrence_type:
            ...
            # Transformer state is kept flat (3 tensors per block) so it survives
            # Lightning/DDP collation: [state_mask, K, V] per block.
            assert len(state) == 3 * len(self.blocks), (
                f"Length of state {len(state)} did not match 3x length of blocks {len(self.blocks)}"
            )
            state = [(state[3 * i], (state[3 * i + 1], state[3 * i + 2]))
        ...
        if "lstm" not in self.recurrence_type:
            state_out = [t for s in state_out for t in (s[0], s[1][0], s[1][1])]
    ...
    def initial_state(self, batchsize):
        ...
        return [t for s in ret for t in (s[0], s[1][0], s[1][1])]
...
class ResidualRecurrentBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert not inject_condition, "condition injection is not used by ROCKET-2"
        ...
        if recurrence_type in ["multi_layer_lstm", "multi_layer_bilstm"]:
            ...
            raise NotImplementedError(recurrence_type)
