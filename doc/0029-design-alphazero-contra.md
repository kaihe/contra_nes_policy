...
counts supervise the policy head; terminal success supervises the value head as specified
by [0030](0030-design-terminal-value-progress-heads.md); decoded emulator state supervises
all auxiliary heads. Repeated search,
...
| value | terminal task success | binary cross-entropy | leaf evaluation |
...
## The value predicts terminal success under searched play

For a non-terminal state, value means the probability that searched play completes the
declared task. Completed episodes assign their binary terminal outcome to every sampled
decision. MCTS backs up terminal success or the network prediction at a non-terminal leaf;
it does not add cumulative `mc_search` reward to this probability. Dense reward remains a
separate diagnostic. [0030](0030-design-terminal-value-progress-heads.md) owns the precise
outcome, progress-label, loss, masking, and evaluation contracts.
...
finish the episode and assign its terminal outcome to every sample
