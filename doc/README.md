...
|---|---|---|
| [0010 Dropout as a gated probe](0010-dropout-regularization.md) | Proposed | the CE-optimal checkpoint plays 12.9 pp *worse* than the overfit one, so val CE is anti-correlated with play; run dropout 0.1/0.2/0.3 on D8 only as a falsifiable probe, gated on the overfit rise actually shrinking, and build tail CE (val CE on non-`R` frames) as the replacement offline proxy. Rejects model scaling — train CE is already 0.051. |
