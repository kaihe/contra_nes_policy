...
|---|---|---|
| [0013 Scaling grid: core size x data](0013-scaling-grid.md) | Proposed | GRPO is now eliminated too (eval 0014: Spread-only binary reward finished at 9.5%, *exactly* its init), leaving capacity and data-beyond-2,500-episodes. Cross **6 core sizes (1.6M–101M, 0.14x–7.9x current)** with the **5 nested prefixes (762–9,900 episodes)** of `boss-spread-10k-v1`, boss-only and Spread-only. Enabled by caching the frozen encoder's tokens: the encoder is **74% of a step** and the core is 14 ms of it, so 790 MB of bf16 turns a 1-hour cell into 3–14 minutes and the grid into ~4 GPU-hours. Reopens the model scaling 0010 rejected, because that rejection was measured at 666 boss episodes and this is 15x. **Predicts the model axis is flat** (§4) — over-parameterized ~80x by tokens/param even at 40k. All 9,900 traces share **one start state**, so the 57-task mixed-v2 transfer set is co-primary with the release's own holdout. 20k/40k predeclared, blocked on data. |
