...
|---|---|---|
| [0011 Is boss RL-solvable?](0011-boss-grpo.md) | Proposed | a feasibility verdict, not a metric change: 10 h wall-clock of graded-reward GRPO, boss rollout share 1.0, finishing the run defunded at update 61. Grading already cut wasted rollout budget 51% → 27%. Read on two axes — train-start vs held-out boss success — because "RL can fit boss" and "RL generalizes on boss" are different claims; a negative needs a plateau, not a low endpoint. Other families are explicitly out of scope. Supersedes 0008. |
| [0010 No offline CE proxy](0010-dropout-regularization.md) | Implemented | the CE-optimal checkpoint plays 12.9 pp *worse* than the overfit one, and the proposed fix — CE on the non-`R` frames — turns out to be a constant 2.1–2.3× multiple of total CE at both ends of training and across 4× data. Held-out imitation loss degrades uniformly while play improves, so *no* reweighting of CE can proxy closed-loop success. Defers the dropout sweep (runbook in §5), rejects model scaling — train CE is already 0.051. |
